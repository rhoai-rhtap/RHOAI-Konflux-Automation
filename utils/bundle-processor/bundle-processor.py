...
class bundle_processor:
    ...
    def patch_related_images(self):
        ...
        env_object = jsonupdate_ng.updateJson({'env': env_list}, {'env': self.merged_image_list}, meta={'listPatchScheme': {'$.env': {'key': 'name'}}}) #, 'keyType': 'partial', 'keySeparator': '@'
        self.csv_dict['spec']['install']['spec']['deployments'][0]['spec']['template']['spec']['containers'][0][
            'env'] = env_object['env']
        relatedImages = []
        ...
        relatedImages += [{'name': image['name'].replace('RELATED_IMAGE_', '').lower(), 'image': image['value']} for image in self.merged_image_list]
